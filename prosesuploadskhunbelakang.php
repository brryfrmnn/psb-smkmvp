...
if (isset($_FILES['file'])) 
	{ 
		$namaFile = $_SESSION['session_id'].'-SKHUN-BLKG-'.$_SESSION['username'].'-'.$_FILES['file']['name']; // berisi nama file yang sedang di file
		...
		if($ukuranFile > 0 or $errorFile == 0)
			{
				if($tipeFile == "image/jpeg" or $tipeFile == "image/jpg")
				{
					...
					if ($ketemu) {
						if($proses=$mysqli->prepare("UPDATE mvp_file_calon_siswa set file_calon_siswa_skhun_belakang=? where user_id='$id'"))
						{
					...
					{
						if($proses=$mysqli->prepare("INSERT INTO mvp_file_calon_siswa (file_calon_siswa_skhun_belakang,user_id, created_at) values (?,'$id', CURRENT_TIMESTAMP)")){
							$proses->bind_param("s", $namaFile);
							$proses->execute();
						}
						else
